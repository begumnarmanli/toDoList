document.addEventListener('DOMContentLoaded', () => {
        ...
        if (Notification.permission === 'default') {
            ...
            Notification.requestPermission().then(permission => {
                if (permission === 'granted') {
                    ...
                    setTimeout(() => {
                        if (confirm('Alarm sistemi için bildirim izni gerekli! Tekrar denemek ister misiniz?')) {
            ...
            setTimeout(() => {
                if (confirm('Alarm sistemi için bildirim izni gerekli! Tarayıcı ayarlarından izin vermeniz gerekiyor. Tekrar denemek ister misiniz?')) {
                    // Tarayıcı ayarlarını aç
                    if (confirm('Tarayıcı ayarlarını açmak ister misiniz?')) {
                        // Chrome için
                        if (navigator.userAgent.includes('Chrome')) {
                            alert('Chrome ayarları → Gizlilik ve güvenlik → Site ayarları → Bildirimler → Bu site için izin ver');
                        }
                    }
                    // Tekrar izin iste
                    Notification.requestPermission();
    ...
    function createTaskCardElement(customer, index) {
        ...
        deleteTaskBtn.addEventListener('click', () => {
            showConfirmModal('Bu görevi silmek istediğinizden emin misiniz?', () => {
    ...
    function openEditModal(index) {
        try {
            ...
            if (editProjectTrigger) editProjectTrigger.textContent = customer.project || 'Proje Türü Seçin';
    ...
    function openDetailModal(customerName, taskCard) {
        ...
        document.getElementById('detailTitle').textContent = `${customerName} - Yapılacaklar`;
    ...
    function showDeadlineNotification(customer, message) {
        ...
        // Console'a log
        console.log(`🔔 ALARM: ${customer.name} - ${customer.project} - ${message}`);
    }
